...
## [Day 22](https://www.adventofts.com/events/2024/22)

As expected, today is the day from where the whole journey starts to get gnarly! Writing a JSON parser in a traditional programming language is itself a complex task, but doing the same in TypeScript is a whole new level of complexity! It would be crucial to break down the problem into smaller parts and then combine them to form the final solution. I would be helping you get started with this seemingly impossible task in the introduction, and would highly recommend you to try and solve the problem on your own before looking at the final solution!

Parsing JSON directly from a string would be a bit too complex, and can lead to a number of edge cases due to the inconsistent formatting of JSON strings. Moreover the recursive nature of JSON objects and arrays would trouble you a lot if you start to parse it directly (matching brackets and quotes appropiately!). Thus, we would first tokenize the whole JSON string into smaller parts and then parse the tokens to form the final JSON object.

A token in languages is a single unit of meaning, and in the case of JSON, it would be the smallest unit of the JSON string that has a specific meaning. For example, the tokens in JSON would be the opening and closing braces, brackets, commas, colons, strings, numbers, and booleans. We would first tokenize the JSON string into these smaller parts and then parse the tokens to form the final JSON object. The tokens have have some value associated with them (like the value of a string, number, or boolean), or they can be just characters or symbols (like the opening and closing braces, brackets, commas, and colons).

In our case, the tokens that JSON supports are:
...
type OpenBraceToken = { type: "OpenBrace" };
type CloseBraceToken = { type: "CloseBrace" };
type OpenSquareBracketToken = { type: "OpenSquareBracket" };
type CloseSquareBracketToken = { type: "CloseSquareBracket" };
type CommaToken = { type: "Comma" };
type ColonToken = { type: "Colon" };
type NilToken = { type: "Nil" };

type StringToken<T extends string> = { type: "String"; value: T };
type NumberToken<T extends number> = { type: "Number"; value: T };
type BooleanToken<T extends boolean> = { type: "Boolean"; value: T };
```

Next stage in such analysis is the design of the tokenizer! A tokenizer is a program that breaks the input string into tokens. In our case, the tokenizer would take a JSON string as input and return an array of tokens. For example, the JSON string `"{\"name\": \"Alice\"}"` would be tokenized into an array of tokens `[OpenBraceToken, StringToken<"name">, ColonToken, StringToken<"Alice">, CloseBraceToken]`.

The tokenizer is a linear program that just matches the starting of the current input string with the possible tokens and then returns the token and the rest of the string. The tokenizer would be a recursive program that keeps on matching the input string with the possible tokens until the input string is exhausted. The final solution would look something like:
...
type Tokenize<S extends string> =
  Trim<S> extends `{${infer Rest}`
    ? [OpenBraceToken, ...Tokenize<Rest>]
  : Trim<S> extends `}${infer Rest}`
    ? [CloseBraceToken, ...Tokenize<Rest>]
  : Trim<S> extends `[${infer Rest}`
    ? [OpenSquareBracketToken, ...Tokenize<Rest>]
  : Trim<S> extends `]${infer Rest}`
    ? [CloseSquareBracketToken, ...Tokenize<Rest>]
  : Trim<S> extends `,${infer Rest}`
    ? [CommaToken, ...Tokenize<Rest>]
  : Trim<S> extends `:${infer Rest}`
    ? [ColonToken, ...Tokenize<Rest>]
  : Trim<S> extends `nil${infer Rest}`
    ? [NilToken, ...Tokenize<Rest>]
  : Trim<S> extends `true${infer Rest}`
    ? [BooleanToken<true>, ...Tokenize<Rest>]
  : Trim<S> extends `false${infer Rest}`
    ? [BooleanToken<false>, ...Tokenize<Rest>]
  : Trim<S> extends `"${infer Str}"${infer Rest}`
    ? [StringToken<Str>, ...Tokenize<Rest>]
  : Trim<S> extends `'${infer Str}'${infer Rest}`
    ? [StringToken<Str>, ...Tokenize<Rest>]
  : Trim<S> extends `${infer Num extends number}${infer Rest}`
    ? [NumberToken<Num>, ...Tokenize<Rest>]
  : []
```
...
- The `Trim` utility type is used to remove the leading whitespaces, tabs, and newlines from the input string. We do not care about trimming the back here, as the tokenizer wouldn't be matching the tokens from the back!
- For the character tokens like `[]{},:`, we just match the first character of the input string and return the appropriate token. We then recursively call the `Tokenize` type with the rest of the string.
- For the `StringToken`, we have two branches: one that matches the string enclosed in double quotes and the other that matches the string enclosed in single quotes.
- We use a `infer Num extends number` typeguard while matching the `NumberToken` to ensure that the matched number is of the type number.
- Constant tokens such as `nil`, `true`, and `false` are matched directly with the input string. It is also important to not that these tokens are matched before the `StringToken` and `NumberToken` to avoid any conflicts in the precedence of the tokens.

With this type is place, can you try to write a type that takes in an array of tokens (`Token[]`) and returns the parsed JSON object? (You might need to handle the parsing for three fundamental kinds of JSON values: objects, arrays, and literals!)

<details>
<summary>Solution</summary>

As described in the hint above, we will be definning a number of types to parse the JSON object from the tokens. All of these parsing types would return the `ParseResult` type, which is a tuple of the parsed JSON object and the rest of the tokens remaining to be parsed. The broadest type would be the `Parser` type (which represents the JSON object), and would call the `ParseObject`, `ParseArray`, and `ParseLiteral` types to parse the JSON object, array, and literals respectively based on the first input token. (We will define all these types as empty types for now, and then fill them up one by one!)

```typescript showLineNumbers
type ParseResult<T, Left extends unknown[]> = [T, Left];

type Parser<L extends Token[]> = L extends [OpenBraceToken, unknown[]]
  ? ParseObject<L>
  : L extends [OpenSquareBracketToken, unknown[]]
    ? ParseArray<L>
    : ParseLiteral<L>;

type ParseObject<L> = {};
type ParseArray<L> = [];
type ParseLiteral<L> = NilToken;
```

Let's start with the easiest type, the `ParseLiteral` type, which would parse the literals like strings, numbers, booleans, and nulls. The type would match the first token with the possible literal tokens and return the parsed value and the rest of the tokens.

```typescript showLineNumbers
type ParseLiteral<L extends Token[]> = L extends [infer Cur, ...infer Rest]
  ? Cur extends StringToken<infer Str>
    ? ParseResult<Str, Rest>
    : Cur extends NumberToken<infer Num>
      ? ParseResult<Num, Rest>
      : Cur extends BooleanToken<infer Bool>
        ? ParseResult<Bool, Rest>
        : Cur extends NilToken
          ? ParseResult<null, Rest>
          : never
  : never;
```

We will also be writing a couple of simple tests with each type to ensure that the parsing is working as expected.

```typescript showLineNumbers
type Test_PL1 = ParseLiteral<[StringToken<"Hello">, CommaToken]>; // Expected: ["Hello", [CommaToken]]
type Test_PL2 = ParseLiteral<[NumberToken<42>, CommaToken]>; // Expected: [42, [CommaToken]]
type Test_PL3 = ParseLiteral<[BooleanToken<true>, CommaToken]>; // Expected: [true, [CommaToken]]
type Test_PL4 = ParseLiteral<[NilToken, CommaToken]>; // Expected: [null, [CommaToken]]
```

Next, we will defint the `ParseArray` type, which would parse the JSON arrays. The type would match the first token with the opening square bracket and then recursively call the `Parser` type to parse the array elements. The type would keep on parsing the array elements until it encounters the closing square bracket (in the `ParseArrayHelper` type).

```typescript showLineNumbers
type ParseArray<L extends Token[]> = L extends [
  OpenSquareBracketToken,
  ...infer Rest,
]
  ? ParseArrayHelper<Rest, []>
  : never;

type ParseArrayHelper<L extends Token[], Acc extends unknown[]> = L extends [
  CloseSquareBracketToken,
  ...infer Rest extends Token[],
]
  ? ParseResult<Acc, Rest>
  : L extends [CommaToken, ...infer Rest extends Token[]]
    ? ParseArrayHelper<Rest, Acc>
    : Parser<L> extends ParseResult<infer Ele, infer Rest>
      ? ParseArrayHelper<Rest, [...Acc, Ele]>
      : never;
```

You might note in the implementation of `ParseArrayHelper`, we just ignore the occurence of a `CommaToken` and carry out the parsing recursively, without actually enforcing that two elements must be seperated by a `,`. You may try to add that additional constraint as well, but given the complexity of the challenge and the good faith that only valid JSON objects would be provided to us, I decided to skip that. Also, here are some tests for the `ParseArray` type to ensure it's sanity:

```typescript showLineNumbers
type Test_PA1 = ParseArray<
  [OpenSquareBracketToken, CloseSquareBracketToken, CommaToken]
>; // Expected: [[], [CommaToken]]
type Test_PA2 = ParseArray<
  [OpenSquareBracketToken, NumberToken<1>, CloseSquareBracketToken]
>; // Expected: [[1], []]
type Test_PA3 = ParseArray<
  [
    OpenSquareBracketToken,
    NumberToken<1>,
    CommaToken,
...
>; // Expected: [[1, 2], [CommaToken]]
type Test_PA4 = ParseArray<
  [
    OpenSquareBracketToken,
    StringToken<"Hello">,
    CommaToken,
...
>; // Expected: [["Hello", "World"], [CommaToken]]
```

Finally, we will define the `ParseObject` type, which would parse the JSON objects. The parsing of the same would be a lot like that of `ParseArray`, but we would need to ensure that each key value pair is of the form `String/Number : JSONElement`. We make use of the `ParseRecord` type to parse the key value pairs and then recursively call the `ParseObjectHelper` type to parse the rest of the object.

```typescript showLineNumbers
type ParseObject<L extends Token[]> = L extends [OpenBraceToken, ...infer Rest]
  ? ParseObjectHelper<Rest, {}>
  : never;

type ParseRecord<L extends Token[]> = L extends [
  StringToken<infer Key>,
  ColonToken,
  ...infer Rest extends Token[],
]
  ? Parser<Rest> extends ParseResult<infer Value, infer Next>
    ? ParseResult<{ [K in Key]: Value }, Next>
    : never
  : L extends [
        NumberToken<infer Key>,
        ColonToken,
        ...infer Rest extends Token[],
      ]
    ? Parser<Rest> extends ParseResult<infer Value, infer Next>
...
type ParseObjectHelper<
  L extends Token[],
  Acc extends Record<string | number, unknown>,
> = L extends [CloseBraceToken, ...infer Rest extends Token[]]
  ? ParseResult<Acc, Rest>
  : L extends [CommaToken, ...infer Rest extends Token[]]
    ? ParseObjectHelper<Rest, Acc>
    : ParseRecord<L> extends ParseResult<infer Ele, infer Rest>
      ? ParseObjectHelper<Rest, Acc & Ele>
      : never;
```

As per the norm, here are some tests for the `ParseObject` type:

```typescript showLineNumbers
type Test_PO1 = ParseObject<[OpenBraceToken, CloseBraceToken, CommaToken]>; // Expected: [{}, [CommaToken]]
type Test_PO2 = ParseObject<
  [
    OpenBraceToken,
    StringToken<"name">,
    ColonToken,
...
>; // Expected: [{ name: "Alice", age: 42 }, [CommaToken]]
```

Finally, we can combine it all together to form the final `Parse` type, which would take the JSON string as input and return the parsed JSON object.

```typescript showLineNumbers
type Parse<S extends string> = Parser<Tokenize<S>>[0];
```

But, we run into a new problem this time: as soon as we instantiate the `Parse` type, TypeScript throws a `Type instantiation is excessively deep and possibly infinite` error! This is because the recursive nature of the types is too complex for TypeScript to handle, and as the error says: JSON can be infinitely nested!
